...
def predict():
    ...
    try:
        ...
        res = {}
        res["result"] = (
            sp.translate(result, data.get("target_language"))
            if data.get("target_language")
            else sp.translate(result)
        )

        if data.get("speech_request"):
            res["audio_b64"] = (
                sp.text_to_speech(res["result"], data.get("target_language"))
                if data.get("target_language")
                else sp.text_to_speech(res["result"])
            )
